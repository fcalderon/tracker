...
let Session = props => {
  return [
    <span className="navbar-text mr-1" key={'user-name'}>
      {props.token ? props.token.tokenWrapper.user.name : ''} |
    </span>,
    <NavItem key={'logout-button'}>
      <div className={'navbar-text'} onClick={() => handleLogOut(props)}>
        Log out
      </div>
    </NavItem>,
  ];
...
const Nav = props => {
  ...
  if (props.token) {
    ...
    sessionInfo = [
      ...
      <NavItem key={'sign-up-nav-item'}>
        <NavLink to="/signUp" href="#" className="nav-link">
          Sign Up
        </NavLink>
      </NavItem>,
    ];
  ...
  return (
    <nav className="navbar navbar-dark navbar-expand-lg bg-dark navbar-expand">
      ...
      <ul className="navbar-nav">{sessionInfo}</ul>
    </nav>
  );
};
